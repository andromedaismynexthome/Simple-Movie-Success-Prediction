...
def home(request):
    ...
    paginator = Paginator(movies, 4)  # Show 4 contacts per page
...
def details(request, id):
    ...
    if result < 0.2:
        pr = "Disaster...dont buy..be freeloader"
    elif result >= 0.2 and result < 0.4:
        pr = "Flop..think once to book ticket"
        ...
        pr = "Blockbuster...enjoy your movie"
...
def add_movies(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect('/add_movies?submitted=True')
    else:
        form = MoviesForm()
        if 'submitted' in request.GET:
                 submitted = True
    
    return render(request, 'movies/add_movies.html', {'form':form ,'submitted':submitted})
