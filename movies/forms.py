...
class MoviesForm(ModelForm):
    class Meta:
        model = Movies
        fields = {'name','year','imdb_meta','genres','runtime','director','yt_trailer_views','ph_credit','yt_trailer_like'}
        labels = {
            'name':'',
            'year':'',
            #'link':'',
        ...
        widgets={
            ...
            'genres':forms.CheckboxSelectMultiple(attrs={'class':'form-select','placeholder':''}),
            #'stars':forms.RadioSelect(attrs={'class':'form-select','placeholder':''}),
